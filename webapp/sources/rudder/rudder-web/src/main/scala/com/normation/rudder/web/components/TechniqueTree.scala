  ...
  def tree(): NodeSeq = {
    ...
    (for {
      ...
      case e: EmptyBox =>
        ...
        logger.error(msg, e)
        (new JsTreeNode {
          override def body:     scala.xml.Elem                      = <span class="error">Can not find dependencies. <span class="errorDetails">{
            (e ?~! msg).messageChain
          }</span></span>
          override def children: scala.collection.immutable.Nil.type = Nil
        }).toXml
    }
  }
  ...
  private[this] def ruleNode(id: RuleId): JsTreeNode = {
    ruleRepository.get(id).toBox match {
      ...
      case e: EmptyBox =>
        ...
        new JsTreeNode {
          override def body:     scala.xml.Elem                      = <span class="error">Can not find Rule details for id {
            id
