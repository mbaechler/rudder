...
class CommitAndDeployChangeRequestServiceImpl(
  ...
  private[this] def isMergeableConfigurationChangeRequest(changeRequest: ConfigurationChangeRequest): Boolean = {
    ...
    final case object CheckRule extends CheckChanges[Rule] {
      ...
      def xmlSerialize(rule: Rule): net.liftweb.common.Full[scala.xml.Elem] = Full(xmlSerializer.rule.serialise(rule))
    ...
    case object CheckGroup extends CheckChanges[NodeGroup] {
      ...
      def xmlSerialize(group: NodeGroup): net.liftweb.common.Full[scala.xml.Elem] = Full(xmlSerializer.group.serialise(group))
    ...
    case object CheckGlobalParameter extends CheckChanges[GlobalParameter] {
      ...
      def xmlSerialize(param: GlobalParameter): net.liftweb.common.Full[scala.xml.Elem] = Full(
        xmlSerializer.globalParam.serialise(param)
      )
