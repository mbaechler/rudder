...
class ModificationService(
  ...
  def restoreToEventLog(eventLog: EventLog, commiter: PersonIdent, rollbackedEvents: Seq[EventLog], target: EventLog): Box[GitCommitId] = {
    for {
  ...
  def restoreBeforeEventLog(eventLog: EventLog, commiter: PersonIdent, rollbackedEvents: Seq[EventLog], target: EventLog): Box[GitCommitId] = {
    for {
      commit   <- getCommitsfromEventLog(eventLog)
      rollback <- commit match {
                    case None    =>
