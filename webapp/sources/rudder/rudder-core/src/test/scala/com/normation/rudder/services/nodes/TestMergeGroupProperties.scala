...
class TestMergeGroupProperties extends Specification {
  ...
  implicit class ToTarget(g: NodeGroup) {
    ...
    def toCriterion: CriterionLine = CriterionLine(null, Criterion("some ldap attr", SubGroupComparator(null), null), null, g.id.serialize)
  }
  ...
  val child: NodeGroup       = NodeGroup(NodeGroupId(NodeGroupUid("child")), "child", "", List(childProp), Some(query), true, Set(), true)
  val nodeInfo    =
