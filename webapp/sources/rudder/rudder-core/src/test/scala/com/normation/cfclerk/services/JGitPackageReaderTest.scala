...
class JGitPackageReader_SameRootTest extends JGitPackageReaderSpec {
  override lazy val gitRoot = new File("/tmp/test-jgit-" + DateTime.now().toString())
  override lazy val ptLib   = gitRoot
  override lazy val relativePathArg: Option[String] = None
  def postInitHook():                Unit           = {}
}
