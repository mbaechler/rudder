...
class TechniqueAPIService6(
  ...
  def serialize(technique: Technique, directive: Directive): JValue = restDataSerializer.serializeDirective(technique, directive, None)
...
class TechniqueAPIService14(
  ...
  def getTechniqueWithData(techniqueName: TechniqueName, version: Option[TechniqueVersion], format: TechniqueApi.QueryFormat): ZIO[Any,RudderError,Seq[Json]] = {
    for {
      lib                    <- readDirective.getFullDirectiveLibrary()
