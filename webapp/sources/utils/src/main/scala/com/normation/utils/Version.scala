...
object ParseVersion {
  ...
  def versionChar(c: Char): Boolean   = ascii.canEncode(c) && !(c.isDigit || c.isControl || c.isSpaceChar || separatorChar(c) || c == ':')
  ...
  def noSepPart2[A: P]: P[List[VersionPart.After]] = P(num).map(n => VersionPart.After(Separator.None, PartType.Numeric(n)) :: Nil)
